...
{
	oled.clrScr();
	oled.print("Ohms:" + (String)data.ohms, CENTER, 8);
...
void draw()
{
	...
	oled.print("mosf:" + (String)digitalRead(mosfet), CENTER, 48);
...
void loop()
{
	...
	if (millis() - batTmr > 20)
	{
		...
		if (checkBat(batVoltF) || true)
		{
			PWM = (float)data.watt / maxW * 1023; // считаем значение для ШИМ сигнала
			if (PWM > 1023)
				PWM = 1023; // ограничил PWM "по тупому", потому что constrain сука не работает!
